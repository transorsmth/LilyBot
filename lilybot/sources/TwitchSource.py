...
class TwitchSource(DataBasedSource):
    ...
    async def get_token(self):
        ...
        try:
            ...
            Lily_LOGGER.critical(f"Error in {self.full_name} Token Get: {response['message']}")
            self.disabled = True
            return
    ...
    async def request(self, url, *args, headers=None, **kwargs):
        ...
        if response.status == 401:
            if 'WWW-Authenticate' in response.headers:
                Lily_LOGGER.info("Twitch token expired when request made, request new token and retrying.")
    ...
    async def get_new_posts(self):
        ...
        if datetime.datetime.now() > self.expiry_time:
            Lily_LOGGER.info("Refreshing Twitch token due to expiry time")
            await self.get_token()
