...
class Starboard(Cog):
    ...
    def make_config_embed(self, ctx: LilyBotContext, title, config):
    ...
    async def starboard(self, ctx: LilyBotContext):
    ...
    @starboard.command()
    async def add(self, ctx: LilyBotContext, message_id: int, channel: discord.TextChannel = None):
...
class StarboardConfig(db.DatabaseTable):
    ...
    __tablename__ = 'lily_starboard_settings'
...
class StarboardMessage(db.DatabaseTable):
    """Each starboard-related setting"""
    __tablename__ = 'lily_starboard_message'
