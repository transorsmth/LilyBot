...
class SafeRoleConverter(RoleConverter):
    ...
    async def convert(self, ctx: LilyBotContext, argument: str):
        try:
...
class Moderation(Cog):
    ...
    async def run_cross_ban(self, ctx: LilyBotContext, user: discord.User, reason: str):
    ...
    async def warn(self, ctx: LilyBotContext, member: discord.Member, *, reason: str):
    ...
    @command()
    @has_permissions(kick_members=True)
    async def customlog(self, ctx: LilyBotContext, *, reason: str):
    ...
    async def timeout(self, ctx: LilyBotContext, duration: float):
    ...
    async def prune(self, ctx: LilyBotContext, target: typing.Optional[discord.Member], num: int):
    ...
    async def punishments(self, ctx: LilyBotContext):
    ...
    async def ban(self, ctx: LilyBotContext, user_mention: discord.User, *, reason: str = "No reason provided"):
    ...
    @has_permissions(ban_members=True)
    @bot_has_permissions(ban_members=True)
    async def unban(self, ctx: LilyBotContext, user_mention: discord.User, *, reason: str = "No reason provided"):
    ...
    async def kick(self, ctx: LilyBotContext, user_mention: discord.User, *, reason: str = "No reason provided"):
    ...
    @has_permissions(manage_roles=True)
    @bot_has_permissions(manage_permissions=True)
    async def mute(self, ctx: LilyBotContext, member_mentions: discord.Member, *, reason: str = "No reason provided"):
    ...
    @has_permissions(manage_roles=True)
    @bot_has_permissions(manage_permissions=True)
    async def unmute(self, ctx: LilyBotContext, member_mentions: discord.Member, *, reason="No reason provided"):
    ...
    async def unlock(self, ctx: LilyBotContext, channel_mention: discord.TextChannel):
        ...
        await channel_mention.edit(sync_permissions=True)
    ...
    @has_permissions(manage_roles=True)
    @bot_has_permissions(manage_permissions=True)
    async def deafen(self, ctx: LilyBotContext, member_mentions: discord.Member, *, reason: str = "No reason provided"):
    ...
    @has_permissions(manage_roles=True)
    @bot_has_permissions(manage_permissions=True)
    async def undeafen(self, ctx: LilyBotContext, member_mentions: discord.Member, *,
                       reason: str = "No reason provided"):
    ...
    @command()
    async def voicekick(self, ctx: LilyBotContext, member: discord.Member, reason: str = "No reason provided"):
    ...
    @bot_has_permissions(kick_members=True)
    @command()
    async def purgenm(self, ctx: LilyBotContext):
    ...
    async def modlogconfig(self, ctx: LilyBotContext, channel_mentions: discord.TextChannel):
    ...
    @command()
    @has_permissions(administrator=True)
    async def nmconfig(self, ctx, channel_mention: discord.TextChannel, role: discord.Role, *,
                       message):
        ...
        role_name = role.name
        await ctx.send(
            "New Member Channel configured as: {channel}. Role configured as: {role}. "
    ...
    @command()
    @has_permissions(administrator=True)
    async def nmpurgeconfig(self, ctx: LilyBotContext, role: discord.Role, days: int):
    ...
    @has_permissions(manage_messages=True)
    async def post(self, ctx, channel: discord.TextChannel, *, title: str):
        """Posts a message in a channel with a title"""
        embed = discord.Embed(title=title, color=discord.Color.blue())
        msg = await channel.send(embed=embed)
        await ctx.send(
            f"Message sent in {channel.mention}, you can add content to it with &post content {msg.channel.id} {msg.id} content")

    @post.command()
    @has_permissions(manage_messages=True)
    async def content(self, ctx, channel_id: int, message_id: int, *, content: str):
        """Adds content to a message"""
        channel = ctx.guild.get_channel(channel_id)
        message = await channel.fetch_message(message_id)
        await message.edit(
            embed=discord.Embed(title=message.embeds[0].title, description=content, color=discord.Color.blue()))
        await ctx.send("Message edited!")

    @command()
    @has_permissions(administrator=True)
    async def memberconfig(self, ctx: LilyBotContext, *, member_role: SafeRoleConverter):
        """
    ...
    async def linkscrubconfig(self, ctx: LilyBotContext, *, link_role: SafeRoleConverter):
        """
    ...
    @group(invoke_without_command=True)
    @has_permissions(manage_messages=True)
    async def crossbans(self, ctx: LilyBotContext):
        """Cross ban"""
    ...
    @has_permissions(administrator=True)
    @bot_has_permissions(ban_members=True)
    async def subscribe(self, ctx: LilyBotContext, guild_id: int):
        ...
        if guild:
            ...
            raise BadArgument("LilyBot could not find that guild! Make sure that LilyBot is in that guild!")

    @crossbans.command()
    @has_permissions(administrator=True)
    async def unsubscribe(self, ctx: LilyBotContext, guild_id: int):
        ...
        if int(result.split(" ", 1)[1]) > 0:
            ...
            raise BadArgument("LilyBot could not find a subscription to that guild!")
...
class GuildModLog(db.DatabaseTable):
    """Holds modlog info"""
    __tablename__ = 'lily_modlogconfig'
...
class CrossBanSubscriptions(db.DatabaseTable):
    ...
    __tablename__ = 'lily_cross_ban_subscriptions'
...
class MemberRole(db.DatabaseTable):
    ...
    __tablename__ = 'lily_member_roles'
    __uniques__ = 'guild_id'
...
class NewMemPurgeConfig(db.DatabaseTable):
    ...
    __tablename__ = 'lily_member_purge_configs'
    __uniques__ = 'guild_id'
...
class GuildNewMember(db.DatabaseTable):
    ...
    __tablename__ = 'lily_new_members'
    __uniques__ = 'guild_id'
...
class GuildMessageLinks(db.DatabaseTable):
    ...
    __tablename__ = 'lily_guild_msg_links'
    __uniques__ = 'guild_id'
...
class PunishmentTimerRecords(db.DatabaseTable):
    ...
    __tablename__ = 'lily_punishment_timers'
    __uniques__ = 'id'
