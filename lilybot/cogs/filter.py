...
class Filter(Cog):
    ...
    async def check_dm_filter(self, ctx: LilyBotContext, embed: discord.Embed):
            ...
            if wordfilter.search(member_after.nick) is not None:
                if not reverted:
                    try:
                        ...
                        await member_after.send(f"{member_after.mention}, your nickname in **{member_after.guild}** "
                                                f"contains a banned word but because your permissions outrank LilyBot "
                                                f"it was not reset")
                    reverted = True
    ...
    @guild_only()
    async def filter(self, ctx: LilyBotContext, advanced: bool = False):
    ...
    @filter.command()
    async def add(self, ctx: LilyBotContext, pattern: str, friendly_name=None):
    ...
    @has_permissions(manage_guild=True)
    @filter.command()
    async def edit(self, ctx: LilyBotContext, filter_id: int, pattern):
    ...
    @has_permissions(manage_guild=True)
    @filter.command()
    async def remove(self, ctx: LilyBotContext, filter_id: int):
    ...
    async def dm_config(self, ctx: LilyBotContext, config: bool):
    ...
    async def whitelist(self, ctx: LilyBotContext):
    ...
    async def whitelist_add(self, ctx: LilyBotContext, *, role: discord.Role):
    ...
    async def whitelist_remove(self, ctx: LilyBotContext, *, role: discord.Role):
...
class WordFilter(db.DatabaseTable):
    ...
    __tablename__ = 'lily_word_filters'
...
class WordFilterSetting(db.DatabaseTable):
    ...
    __tablename__ = 'lily_word_filter_settings'
    __uniques__ = 'id'
...
class WordFilterRoleWhitelist(db.DatabaseTable):
    ...
    __tablename__ = 'lily_word_filter_role_whitelist'
    __uniques__ = 'role_id'
