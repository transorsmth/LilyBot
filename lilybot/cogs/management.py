...
class Management(Cog):
    ...
    async def schedulesend(self, ctx: LilyBotContext):
    ...
    @has_permissions(manage_messages=True)
    async def add(self, ctx: LilyBotContext, channel: discord.TextChannel, time, *, content):
        """Allows a message to be sent at a particular time
        Headers are distinguished by the characters `-/-`
        """
        perms = channel.permissions_for(ctx.guild.me)
        if not perms.send_messages:
            raise BadArgument(f"LilyBot does not have permissions to send messages in {channel.mention}")
        try:
    ...
    add.example_usage = """
    `{prefix}schedulesend add #announcments "1/0/1970 0:00:00 GMT" Epoch -/- 00000`: LilyBot will send a message on the unix epoch in #announcments
    """

    @schedulesend.command()
    @has_permissions(manage_messages=True)
    async def delete(self, ctx: LilyBotContext, entry_id: int):
    ...
    @schedulesend.command()
    @has_permissions(manage_messages=True)
    async def list(self, ctx: LilyBotContext):
...
class ScheduledMessages(db.DatabaseTable):
    ...
    __tablename__ = 'lily_scheduled_messages'
