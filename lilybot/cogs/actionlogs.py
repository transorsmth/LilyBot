...
class Actionlog(Cog):
    ...
    async def messagelogconfig(self, ctx: LilyBotContext, channel_mentions: discord.TextChannel):
    ...
    @has_permissions(administrator=True)
    async def memberlogconfig(self, ctx: LilyBotContext):
    ...
    async def setchannel(self, ctx: LilyBotContext, channel: discord.TextChannel):
    ...
    @memberlogconfig.command()
    @has_permissions(manage_guild=True)
    async def toggleping(self, ctx: LilyBotContext):
    ...
    @memberlogconfig.command()
    @has_permissions(manage_guild=True)
    async def setjoinmessage(self, ctx: LilyBotContext, *, template: str = None):
    ...
    @memberlogconfig.command()
    @has_permissions(manage_guild=True)
    async def setleavemessage(self, ctx: LilyBotContext, *, template=None):
    ...
    @memberlogconfig.command()
    @has_permissions(manage_guild=True)
    async def disable(self, ctx: LilyBotContext):
    ...
    @has_permissions(manage_guild=True)
    async def help(self,
                   ctx: LilyBotContext):  # I cannot put formatting example in example_usage because then it trys to format the example
    ...
    async def locknickname(self, ctx: LilyBotContext, member: discord.Member, *, name: str):
        """Locks a members nickname to a particular string, in essence revoking nickname change perms"""
        try:
            await member.edit(nick=name)
        except discord.Forbidden:
            raise BadArgument(f"LilyBot is not elevated high enough to change {member}'s nickname")
        lock = NicknameLock(
            guild_id=ctx.guild.id,
            member_id=member.id,
            locked_name=name,
    ...
    `{prefix}locknickname @Snowplow#5196 LilyBot`: Locks user snowplows nickname to "LilyBot"
    """

    @command()
    @has_permissions(manage_nicknames=True)
    @bot_has_permissions(manage_nicknames=True)
    async def unlocknickname(self, ctx: LilyBotContext, member: discord.Member):
    ...
    `{prefix}unlocknickname @Snowplow#5196`: Removes nickname lock from user LilyBot
    """
...
class GuildMessageLog(db.DatabaseTable):
    ...
    __tablename__ = 'lily_messagelogconfig'
