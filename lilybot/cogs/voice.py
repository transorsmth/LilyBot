...
class Voice(Cog):
    ...
    async def autoptt(self, ctx: LilyBotContext, voice_channel: discord.VoiceChannel, ptt_threshold: int):
    ...
    autoptt.example_usage = """
        `{prefix}autoptt "General #1" 15 - sets up LilyBot to give force Push-To-Talk when more than 15 users joins a voice channel.
    ...
    `{prefix}voicebind "General #1" voice-general-1` - sets up LilyBot to give users  `voice-general-1` when they join voice channel "General #1", which will be removed when they leave.
    """

    @command()
    @bot_has_permissions(manage_roles=True)
    @has_permissions(manage_roles=True)
    async def voiceunbind(self, ctx: LilyBotContext, voice_channel: discord.VoiceChannel):
    ...
    @command()
    @bot_has_permissions(manage_roles=True)
    async def voicebindlist(self, ctx: LilyBotContext):
