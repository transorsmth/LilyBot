...
def dev_check():
    ...
    async def predicate(ctx: LilyBotContext) -> bool:
...
class Reactor:
    ...
    def __init__(self, ctx: LilyBotContext, initial_reactions, *, auto_remove: bool = True, timeout: int = 60):
        """
        ctx: command context
...
async def paginate(ctx: LilyBotContext, pages, *, start: int = 0, auto_remove: bool = True, timeout: int = 60):
    """
...
def bot_has_permissions(**required):
    ...
    def predicate(ctx: LilyBotContext):
...
class DynamicPrefixEntry(db.DatabaseTable):
    ...
    __tablename__ = 'lily_dynamic_prefixes'
