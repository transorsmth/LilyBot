...
class News(Cog):
    ...
    async def startup(self):
        ...
        self.http_source = aiohttp.ClientSession(
            headers={'Connection': 'keep-alive', 'User-Agent': 'LilyBot RSS Feed Reader'})
    ...
    @guild_only()
    async def news(self, ctx: LilyBotContext):
        """Show help for news subscriptions"""
        embed = discord.Embed(title="How to subscribe to News Sources",
                              description="LilyBot has built in news scrapers to allow you to review up to date news"
    ...
    @guild_only()
    async def add(self, ctx: LilyBotContext, channel: discord.TextChannel, source: Source, kind='embed', data=None):
    ...
    @has_permissions(manage_guild=True)
    @guild_only()
    async def remove(self, ctx: LilyBotContext, channel: discord.TextChannel, source: Source, data=None):
    ...
    async def list_sources(self, ctx: LilyBotContext):
    ...
    @guild_only()
    async def list_subscriptions(self, ctx: LilyBotContext, channel: discord.TextChannel = None):
    ...
    @news.command()
    @dev_check()
    async def restart_loop(self, ctx: LilyBotContext):
    ...
    @news.command()
    @dev_check()
    async def next_run(self, ctx: LilyBotContext):
    ...
    @news.command()
    @dev_check()
    async def get_exception(self, ctx: LilyBotContext):
