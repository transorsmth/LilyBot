...
class Roles(Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        for command in self.giveme.walk_commands():
            ...
            async def givemeautopurge(self, ctx: LilyBotContext):
    ...
    @staticmethod
    async def safe_message_fetch(ctx: LilyBotContext, menu=None, channel: discord.TextChannel = None,
                                 message_id: int = None):
    ...
    async def ctx_purge(self, ctx: LilyBotContext):
    ...
    async def giveme(self, ctx: LilyBotContext, *, roles):
    ...
    async def purge(self, ctx: LilyBotContext):
    ...
    async def add(self, ctx: LilyBotContext, *, name: str):
    ...
    @bot_has_permissions(manage_roles=True)
    @has_permissions(manage_guild=True)
    async def create(self, ctx: LilyBotContext, *, name: str):
    ...
    @giveme.command()
    @bot_has_permissions(manage_roles=True)
    async def remove(self, ctx: LilyBotContext, *, roles):
    ...
    @bot_has_permissions(manage_roles=True)
    @has_permissions(manage_guild=True)
    async def delete(self, ctx: LilyBotContext, *, name: str):
    ...
    @bot_has_permissions(manage_roles=True)
    async def list_roles(self, ctx: LilyBotContext):
    ...
    @bot_has_permissions(manage_roles=True)
    @has_permissions(manage_guild=True)
    async def removefromlist(self, ctx: LilyBotContext, *, name: str):
        """Deletes and removes a giveable role."""
    ...
    @bot_has_permissions(manage_roles=True, embed_links=True)
    @has_permissions(manage_roles=True)
    async def give(self, ctx: LilyBotContext, member: discord.Member, *, role: discord.Role):
    ...
    @bot_has_permissions(manage_roles=True, embed_links=True)
    @has_permissions(manage_roles=True)
    async def take(self, ctx: LilyBotContext, member: discord.Member, *, role: discord.Role):
    ...
    async def update_role_menu(self, ctx: LilyBotContext, menu):
        ...
        for entry in menu_entries:
            ...
            menu_embed.add_field(name=f"\u200b", value=f"{entry.reaction}: {role.mention}", inline=False)
    ...
    @has_permissions(manage_roles=True)
    @guild_only()
    async def rolemenu(self, ctx: LilyBotContext):
    ...
    @has_permissions(manage_roles=True)
    @guild_only()
    async def createmenu(self, ctx: LilyBotContext, channel: discord.TextChannel, *, name: str):
        ...
        e.set_footer(text=f'Menu id: {message.id}, Triggered by ' + escape_markdown(ctx.author.display_name))
        await ctx.send(embed=e)
   ...
   -----To target a custom message use this format-----
    ...
    @has_permissions(manage_roles=True)
    @guild_only()
    async def refresh(self, ctx: LilyBotContext, message_id: int, ):
        """Refreshes a role menu"""
        menu_return = await RoleMenu.get_by(guild_id=ctx.guild.id, message_id=message_id)
        if not len(menu_return):
            raise BadArgument(f"No role menu found with message ID {message_id}")
        menu = menu_return[0]
        await self.update_role_menu(ctx, menu)
        await ctx.send(f"I updated the role menu {menu.name}")

    @rolemenu.command(aliases=['updateall'])
    @bot_has_permissions(manage_roles=True, embed_links=True)
    @has_permissions(manage_roles=True)
    @guild_only()
    async def refreshall(self, ctx: LilyBotContext):
        """Refreshes a role menu"""
        menu_return = await RoleMenu.get_by(guild_id=ctx.guild.id)
        await ctx.send(f"Processing {len(menu_return)} role menus")
        if not len(menu_return):
            raise BadArgument(f"No role menu found in guild {ctx.guild.name}")
        for menu in menu_return:
            print(menu.name)
            await self.update_role_menu(ctx, menu)

        await ctx.send(f"Updated {len(menu_return)} role menus")

    @rolemenu.command()
    @has_permissions(manage_roles=True)
    @guild_only()
    async def synctogiveme(self, ctx: LilyBotContext):
        results = await ReactionRole.get_by(guild_id=ctx.guild.id)
        for result in results:
            ...
            if not candidates:
                ...
                continue
            await GiveableRole.from_role(role).update_or_add()
        await ctx.send(f"Add {len(results)} roles to giveme")

    @rolemenu.command(aliases=["del"])
    @bot_has_permissions(manage_roles=True, embed_links=True)
    @has_permissions(manage_roles=True)
    @guild_only()
    async def delrole(self, ctx: LilyBotContext, channel: typing.Optional[discord.TextChannel], message_id: int,
                      role: discord.Role):
...
class ReactionRole(db.DatabaseTable):
    ...
    __tablename__ = 'lily_reaction_roles'
...
class GiveableRole(db.DatabaseTable):
    ...
    __tablename__ = 'lily_giveable_roles'
    __uniques__ = 'role_id'
...
class MissingRole(db.DatabaseTable):
    ...
    __tablename__ = 'lily_missing_roles'
...
class TempRoleTimerRecords(db.DatabaseTable):
    ...
    __tablename__ = 'lily_temp_role_timers'
    __uniques__ = 'id'
